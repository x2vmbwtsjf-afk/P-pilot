...
export default function Racks() {
  ...
  return (
    <div className="fade-in">
      ...
      <input className="input" placeholder="Search racks..." value={search} onChange={e => setSearch(e.target.value)} style={{ marginBottom: '1.25rem', maxWidth: 340 }} />
              ...
              <div key={rack.id} className="card" style={{ padding: '1.25rem', cursor: 'pointer' }} onClick={() => navigate(`rack/${rack.id}`)}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '0.75rem' }}>
                  ...
                  <div style={{ display: 'flex', gap: '0.35rem' }} onClick={e => e.stopPropagation()}>
                    <button className="btn-icon" onClick={() => { setEditing(rack); setShowModal(true); }}>
                      <EditIcon />
                    </button>
                    <button className="btn-icon" style={{ color: '#ff4d4d', borderColor: 'rgba(255,77,77,0.3)' }} onClick={() => handleDelete(rack.id)}>
                      <TrashIcon />
                    </button>
                  </div>
                </div>
                ...
                <div style={{ marginBottom: '0.75rem' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.3rem' }}>
                    ...
                    <span style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', fontWeight: 600 }}>{usedU}/{rack.totalU}U</span>
                  </div>
    ...
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal-box">
        ...
        <form onSubmit={submit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          <Field label="Name *"><input className="input" required value={form.name} onChange={set('name')} placeholder="e.g. RACK-A1" /></Field>
          <Field label="Location"><input className="input" value={form.location} onChange={set('location')} placeholder="e.g. Row A, DC-1" /></Field>
