...
const DEVICE_TYPES: DeviceType[] = ['server','switch','router','storage','pdu','patch-panel','ups','other'];
...
export default function Devices() {
  ...
  const filtered = devices.filter(d => {
    ...
    const matchQ = !q || d.name.toLowerCase().includes(q) || d.serial.toLowerCase().includes(q) || (d.ip ?? '').includes(q);
  ...
  return (
    <div className="fade-in">
      ...
      <div style={{ display: 'flex', gap: '0.75rem', marginBottom: '1.25rem', flexWrap: 'wrap' }}>
        <input className="input" placeholder="Search name, serial, IP..." value={search} onChange={e => setSearch(e.target.value)} style={{ maxWidth: 280 }} />
      ...
      <div className="modal-box">
        ...
        <form onSubmit={submit} style={{ display: 'flex', flexDirection: 'column', gap: '0.85rem' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.85rem' }}>
            <Field label="Name *"><input className="input" required value={form.name} onChange={set('name')} placeholder="e.g. web-srv-01" /></Field>
            <Field label="Type">
            ...
            <Field label="Manufacturer"><input className="input" value={form.manufacturer} onChange={set('manufacturer')} placeholder="Dell, HP..." /></Field>
            <Field label="Model"><input className="input" value={form.model} onChange={set('model')} placeholder="PowerEdge R640" /></Field>
          </div>
